...
============================================================
REGLA -1 — VERIFICACIÓN PREVIA OBLIGATORIA (antes de generar)
============================================================

Antes de generar CUALQUIER archivo, aplica este checklist interno.
Si la respuesta a alguna pregunta es "sí", DEBES actuar en
consecuencia. NO generes código sin haberlo verificado.

1. ¿El DDL incluye columnas llamadas id, created_at, updated_at,
   version (o cualquier variante como creation_date,
   last_modified, etc.)?
   -> SIEMPRE se omiten de la entidad: las aporta BaseEntity.
   -> SIEMPRE se omiten del DTO: las aporta BaseDTO.
   -> Si el nombre en el DDL difiere del esperado por BaseEntity,
   avisa al usuario al final del archivo (comentario), pero
   NO las incluyas en la entidad ni en el DTO.

2. ¿Se me ha pedido generar el módulo completo?
   -> SIEMPRE debes generar DOS archivos de service:
   <Entity>Service.java (interfaz)
   <Entity>ServiceImpl.java (implementación)
   -> NUNCA generes un único archivo de service.
   -> NUNCA generes la implementación sin la interfaz.
   -> NUNCA generes la interfaz sin la implementación.

3. ¿El DDL declara PK o FK?
   -> PK: nunca se redeclara (la aporta BaseEntity).
   -> FK: se convierte en relación JPA (ver REGLA 1).

4. ¿Hay columnas de auditoría con nombres distintos al estándar
   (por ejemplo "creation_date" en vez de "created_at")?
   -> Genera la entidad SIN esas columnas (las aporta BaseEntity
   con sus propios nombres), y añade al final del archivo un
   comentario indicando la discrepancia con el DDL y la
   migración sugerida.
...
en "core.base" y son reutilizadas por todos los módulos.
    ...
    └── modules
        └── <module_name>
            ...
            │   └── <Entity>Repository
            └── service
                ├── <Entity>Service           <- INTERFAZ
                └── <Entity>ServiceImpl       <- IMPLEMENTACIÓN

ATENCIÓN: en cada módulo existen SIEMPRE DOS archivos de service:
1. <Entity>Service.java      -> interfaz pública
2. <Entity>ServiceImpl.java  -> clase anotada con @Service

NUNCA se genera uno sin el otro. NUNCA se fusionan en un solo
archivo.
...
repository, service (interfaz + impl, DOS ARCHIVOS) y controller.
...
3. DESCARTAR las columnas de auditoría/ID que ya aporta
   BaseEntity. Estas columnas NO se traducen a campos Java en la
   entidad ni en el DTO:

   COLUMNAS PROHIBIDAS EN ENTIDAD Y DTO (las aporta BaseEntity /
   BaseDTO):
   -----------------------------------------------------------------
   | Nombre en DDL                          | Origen             |
      |----------------------------------------|--------------------|
   | id                                     | BaseEntity         |
   | created_at / creation_date / created   | BaseEntity         |
   | updated_at / update_date / modified_at | BaseEntity         |
   | version                                | BaseEntity         |
   -----------------------------------------------------------------

   Estas columnas se ignoran por completo en:
    - La entidad JPA (NO declarar campo ni @Column).
    - El DTO (NO declarar campo ni @Schema).
    - El mapper (no mapear).
    - El repository (no añadir query methods sobre ellas).
    - El service (no exponerlas).

   Si el DDL las declara con otros nombres (p. ej. creation_date,
   last_modified), igualmente se omiten, y se añade un comentario
   al final del archivo de la entidad avisando de la discrepancia
   y proponiendo la migración.

4. Mapear nombre de tabla y columnas:
...
|----------------------------|--------------------|----------------------------|
| uuid (excepto si es id)    | UUID               | (sin @GeneratedValue)      |
...
Columnas detectadas:
- id                        -> OMITIR (BaseEntity)
- created_at                -> OMITIR (BaseEntity)
- updated_at                -> OMITIR (BaseEntity)
- hectares                  -> BigDecimal, not null
- minimum_moisture_threshold -> BigDecimal, not null
...
Salida (entity):
    ...
    public class PlotZone extends BaseEntity {
        ...
        // NO se declaran: id, createdAt, updatedAt, version.
        // Los aporta BaseEntity.
    }
...
Salida (DTO) derivada del mismo DDL:
    ...
    @Schema(description = "Data Transfer Object representing a Plot Zone")
    public class PlotZoneDTO extends BaseDTO {

        @Schema(description = "Surface of the plot in hectares",
                example = "12.5000")
        @NotNull(message = "The hectares is mandatory")
        @Positive(message = "The hectares must be positive")
        private BigDecimal hectares;

        @Schema(description = "Minimum moisture threshold for the plot",
                example = "0.3500")
        @NotNull(message = "The minimum moisture threshold is mandatory")
        @Positive(message = "The minimum moisture threshold must be positive")
        private BigDecimal minimumMoistureThreshold;

        // NO se declara id, createdAt, updatedAt ni version.
        // Los aporta BaseDTO.
    }

Notas:
- id, created_at, updated_at y version NO se declaran NUNCA en la
  entidad (los aporta BaseEntity) ni en el DTO (los aporta
  BaseDTO).
...
"<paquete_base>.modules.<module>.entity":
    ...
    public class Foo extends BaseEntity {
        // SOLO atributos de negocio + relaciones
        // NUNCA id, createdAt, updatedAt, version
    }
...
CAMPOS PROHIBIDOS EN LA ENTIDAD (los aporta BaseEntity):
- id
- createdAt / createdDate / creationDate
- updatedAt / updatedDate / modificationDate
- version

NUNCA declarar estos campos, ni con @Column, ni con @Id, ni con
@GeneratedValue, ni con @Version.
...
Ejemplo canónico:
    ...
    public class Warehouse extends BaseEntity {
        ...
        // NO se declara id, createdAt, updatedAt, version.
    }
...
@Schema. Estructura obligatoria:
    ...
    public class FooDTO extends BaseDTO {
        ...
        // NUNCA id, createdAt, updatedAt ni version.
...
CAMPOS PROHIBIDOS EN EL DTO (los aporta BaseDTO):
- id
- createdAt / createdDate / creationDate
- updatedAt / updatedDate / modificationDate
- version

NUNCA declarar estos campos, ni con @Schema, ni con validaciones,
...
- NUNCA declarar query methods sobre id, createdAt, updatedAt ni
  version (los gestiona BaseEntity / BaseServiceImpl).
...
- NUNCA mapear id, createdAt, updatedAt ni version: BaseMapper
  los ignora por diseño.

============================================================
REGLA 5 — SERVICE (SIEMPRE DOS ARCHIVOS)
============================================================

Por cada entidad se generan EXACTAMENTE DOS archivos dentro de
"<paquete_base>.modules.<module>.service":

    ARCHIVO 1: FooService.java       (INTERFAZ)
    ARCHIVO 2: FooServiceImpl.java   (IMPLEMENTACIÓN anotada con @Service)

NUNCA se genera un solo archivo fusionando ambos.
NUNCA se omite la interfaz.
NUNCA se omite la implementación.
...
ARCHIVO 1 — Interfaz:
    ...
    public interface FooService extends BaseService<FooDTO, UUID> {
        // Métodos extra opcionales si el negocio lo requiere.
        // Si no hay métodos extra, dejar el cuerpo vacío con
        // llaves o un comentario, pero NUNCA omitir la interfaz.
    }

ARCHIVO 2 — Implementación:
...
- Los métodos heredados de BaseServiceImpl ya cubren el CRUD
  básico; no redeclararlos salvo override justificado.

ENTREGA: cuando se pida el módulo completo o el service, la
respuesta DEBE incluir DOS bloques de código separados, uno por
cada archivo, con su package y sus imports completos.
...
- Inyección por constructor del service (la INTERFAZ), pasado a
  super().
...
- NUNCA declarar id, createdAt, updatedAt ni version en entidades
  NI en DTOs (los aportan BaseEntity / BaseDTO).
- NUNCA declarar el campo id en los DTOs (viene de BaseDTO).
- NUNCA mapear id, createdAt, updatedAt ni version en el mapper.
- NUNCA fusionar Service y ServiceImpl en un único archivo.
- NUNCA generar el service sin la interfaz.
- NUNCA generar el service sin la implementación.
...
- ANTES de generar, ejecutar la VERIFICACIÓN PREVIA (REGLA -1):
  descartar columnas de auditoría, confirmar que se generarán DOS
  archivos de service.
...
- OMITIR SIEMPRE de entidad y DTO: id, createdAt, updatedAt,
  version.
...
- DTOs como CLASES con @Data + @EqualsAndHashCode(callSuper=true)
  extendiendo BaseDTO.
- @Schema con description y example en TODOS los DTOs (excepto
  para id/auditoría, que no existen en el DTO).
- Mensajes de validación estandarizados (REGLA 2).
- Mappers MapStruct extendiendo BaseMapper.
- Services extendiendo BaseService / BaseServiceImpl, en DOS
  archivos separados.
...
============================================================
[ ] ¿Ejecuté la VERIFICACIÓN PREVIA (REGLA -1)?
[ ] ¿El DDL incluye id/created_at/updated_at/version? Si sí,
¿los OMITÍ en la entidad Y en el DTO?
[ ] ¿La entidad NO declara id, createdAt, updatedAt ni version?
[ ] ¿El DTO NO declara id, createdAt, updatedAt ni version?
[ ] ¿El mapper NO mapea id, createdAt, updatedAt ni version?
[ ] ¿Generé DOS archivos de service: FooService.java (interfaz)
y FooServiceImpl.java (implementación con @Service)?
[ ] ¿La interfaz del service extiende BaseService<DTO, ID>?
[ ] ¿La implementación extiende BaseServiceImpl<Entity, DTO, ID>
...
[ ] Al entregar el módulo completo, ¿listé explícitamente los
archivos generados (uno por uno) para que el usuario verifique
que están todos?
